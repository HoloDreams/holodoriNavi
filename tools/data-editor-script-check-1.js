...
let fileHandle = null;
let fallbackFileName = "card-data.js";
    ...
    if (window.showOpenFilePicker) {
        try {
            const [handle] = await window.showOpenFilePicker({
                types: [{ description: 'JavaScript Files', accept: { 'application/javascript': ['.js'], 'text/javascript': ['.js'], 'text/plain': ['.js'] } }],
                multiple: false
            });
            fileHandle = handle;
...
function setupFallbackFileInput() {
    const fallbackInput = document.getElementById("fallbackFileInput");
    ...
    fallbackInput.addEventListener("change", async () => {
        ...
        if (!file) return;
        fileHandle = null;
        fallbackFileName = file.name || "card-data.js";
        const text = await file.text();
        parseAndRenderJS(text);
        updateStatus(fallbackFileName);
    });
}
...
async function saveJSFile() {
    try {
        ...
        link.href = url;
        link.download = fallbackFileName || "card-data.js";
        document.body.appendChild(link);
        link.click();
        link.remove();
        URL.revokeObjectURL(url);
        alert("ブラウザの制限で直接上書きできないため、更新済みファイルをダウンロードしました。元のJSファイルに上書きしてください。");
    } catch (err) {
